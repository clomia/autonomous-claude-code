...
이 시스템은 실제로 자율적으로 동작한다. 시작하면 스스로 코드를 작성하고, 파일을 수정하고, 명령을 실행하며, 자기 자신을 개선한다. `--dangerously-skip-permissions`로 실행되므로 모든 작업에 사람의 승인을 요청하지 않는다.

시스템이 수행하는 모든 변경은 Git으로 추적되며, 체크포인트에서 롤백할 수 있다. 그러나 시스템이 작업하는 디렉토리 내의 파일은 언제든 수정될 수 있다는 점을 이해하고 사용해야 한다.
