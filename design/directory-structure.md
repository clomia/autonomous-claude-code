...
**스크립트**:
- `acc = "cli.main:main"` — `uv run acc` 또는 설치 후 `acc`로 실행. 모든 CLI 명령의 엔트리포인트.

**빌드 시스템**: hatchling을 사용한다. `uv sync`로 의존성 설치, `uv run acc`로 실행.
...
**보안**: `.gitignore`에 포함되어 Git에 커밋되지 않는다. `acc configure` 명령으로 생성된다.
...
**동작 방식**:
1. 별도 launchd LaunchAgent로 실행된다 (`com.acc.watchdog.plist`)
...
**수정 주체**: Claude Code (자기개선에 의한 수정), `acc configure` (초기 설정)
...
**사용**: Watchdog이 Supervisor 생존 여부 확인에 사용. `acc status`가 실행 상태 확인에 사용.
...
**소비자**: TUI (실시간 대시보드), `acc status` (상태 출력).
...
시스템 설정 및 설치 관련 파일. `acc configure`와 `acc start` 명령에서 사용된다.

### `launchd/com.acc.supervisor.plist`

Supervisor LaunchAgent 템플릿. `acc start` 명령이 이 템플릿을 복사하여 `~/Library/LaunchAgents/`에 설치한다.
...
### `launchd/com.acc.watchdog.plist`
...
**사용법**: `acc configure` 시 이 매니페스트를 참조하여 Slack 앱 설정을 안내하거나, Slack API로 앱을 자동 생성.
...
CLI 인터페이스. `acc` 명령어의 엔트리포인트와 서브커맨드를 정의한다.
...
**함수**: `main()`
**역할**: `acc` 명령어 엔트리포인트. `pyproject.toml`의 `[project.scripts]`에서 `acc = "cli.main:main"`으로 등록.

**서브커맨드**:
| 커맨드 | 설명 |
|--------|------|
| `acc configure` | 초기 설정. Slack 토큰 입력, 목적 입력, `.env` 생성, Slack 연결 검증 |
| `acc start` | LaunchAgent 설치 + Supervisor 시작. 시스템 가동 |
| `acc stop` | 시스템 중지 + LaunchAgent 제거 |
| `acc restart` | stop → start 순차 실행 |
| `acc status` | 현재 상태 출력. Supervisor 실행 여부, 현재 미션, 세션 상태 |
| `acc tui` | Textual TUI 실행 |
| `acc logs [--follow]` | 로그 출력. `--follow`로 실시간 테일링 |
| `acc inject "<mission>"` | 미션 큐에 수동 주입. missions.json에 추가 |
| `acc reset [checkpoint]` | 지정 체크포인트로 롤백. 기본: 마지막 체크포인트 |
...
**문서 관계**:
- `DESIGN.md`가 루트 문서. 시스템 개요와 각 하위 문서 링크.
- 하위 문서들은 독립적으로 읽을 수 있으나, DESIGN.md에서 전체 맥락을 먼저 파악하는 것을 권장.
