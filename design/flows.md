...
```
Owner              CLI(automata)      Supervisor       Claude Code       State Manager
  |                   |                   |                |                   |
  |  git clone        |                   |                |                   |
  |  cd project       |                   |                |                   |
  |  uv sync          |                   |                |                   |
...
| 2 | CLI | `.env` 생성 | `SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`, `AUTOMATA_RAW_PURPOSE` 저장 |
| 3 | CLI | 설정 검증 | `.env` 파일 존재 + 토큰 유효성 + 목적 입력 비어있지 않음 확인 |
| 4 | CLI | LaunchAgent 설치 | `setup/launchd/com.clomia.automata.supervisor.plist` + `com.clomia.automata.watchdog.plist`를 `~/Library/LaunchAgents/`에 복사. `KeepAlive=true` 설정 |
| 5 | CLI | launchctl bootstrap | `launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.clomia.automata.supervisor.plist` 실행 |
...
| `~/Library/LaunchAgents/com.clomia.automata.supervisor.plist` | 신규 설치 |
| `~/Library/LaunchAgents/com.clomia.automata.watchdog.plist` | 신규 설치 |
...
### 트리거 조건
Watchdog 프로세스가 주기적 heartbeat 확인에서 Supervisor 이상을 감지할 때. Watchdog는 별도 LaunchAgent (`com.clomia.automata.watchdog.plist`)로 실행된다.
...
launchd             Watchdog                            Supervisor
  ...
  |                    |  │  │      -k com.clomia.automata.supervisor            │  │
  |                    |  │  │      (launchd가 재시작)                           │  │
  |                    |  │  │                                                    │  │
  |                    |  │  └────────────────────────────────────────────────────┘  │
  |                    |  │                                                         │
